...
static int
__shmht_insert (struct shmht *h, void *k, size_t key_size,
				  ...
				  void *v, size_t value_size)
{
	...
	int ret = __shmht_insert (h, k, key_size,
				  v, value_size);
...
/****************************************************************************/
int
shmht_update (struct shmht *h, void *k, size_t key_size,
				  void *v, size_t value_size)
{
	struct internal_hashtable *iht = h->internal_ht;
	if (write_lock (iht->semaphore) < 0)
		return -ECANCELED;
	__shmht_remove__ (h, k, key_size);
